...
class TestDownloader(unittest.TestCase):
	...
	def test_get_playlist_info(self):
        ...
        "playlist_with_membership_videos":'https://www.youtube.com/playlist?list=PL0o_zxa4K1BWYThyV4T2Allw6zY0jEumv',
        "clean_playlist" : "https://www.youtube.com/playlist?list=PLTaK2N2T1t_fNmfuQpIig0GYJWgaiSZhu"
        }

        for key, value in playlists.items():
            url = value
            response = self.response_utils.perform_get_request_text(url)
            json_dict = self.response_utils.get_playlist_info(response)
