...
class TestDownloader(unittest.TestCase):
	...
	def test_did_download_fail(self):
		...
		empty_download_dir = os.path.join(all_videos_dir, "Lady guitarist show at Uottawa  فيروز - زهرة المدائن")
	...
	def test_validate_downloaded_videos(self):
		with patch.dict(self.downloader.__dict__, {'channel_path': self.fixtures_downloaded_channel_path}, clear=True):
			failed_downloads = self.downloader.validate_downloaded_videos()
			assert len(failed_downloads) == 5, "The length of the list should be 5 as thats the num of faulty downloads"
